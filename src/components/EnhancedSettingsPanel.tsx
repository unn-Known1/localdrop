        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between p-3 rounded-xl bg-[#111827]"><div><p className="text-white">Detailed Stats</p><p className="text-xs text-gray-400">Show detailed transfer statistics</p></div><button onClick={() => updateSettings({ showDetailedStats: !settings.showDetailedStats })} className={`w-12 h-6 rounded-full transition-colors ${settings.showDetailedStats ? 'bg-blue-500' : 'bg-gray-600'}`}><div className={`w-5 h-5 rounded-full bg-white transition-transform ${settings.showDetailedStats ? 'translate-x-6' : 'translate-x-0.5'}`} /></div></div>
            </div>
          </div>
        </div>
      </div>
